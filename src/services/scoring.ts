...
export const calculateFlightScore = ({
  ...
  const landingBonus: Record<string, number> = {
    Perfect: 1000,
    Good: 500,
    Hard: 100,
    Crash: -500,
...
export const getAchievements = (
  flightData: AchievementCheckData,
): Achievement[] => {
  ...
  if (flightData.maxAltitude >= 40000) {
    achievements.push({
      name: "High Flyer",
      icon: "🚀",
      description: "Reached 40,000 feet",
    });
  }
  ...
  if (flightData.maxSpeed >= 300) {
    achievements.push({
      name: "Speed Demon",
      icon: "⚡",
      description: "Exceeded 300 knots",
    });
  }
  ...
  if (flightData.flightTime >= 3600) {
    achievements.push({
      name: "Endurance",
      icon: "⏱️",
      description: "Flew for over an hour",
    });
  }
  ...
  if (flightData.smoothnessScore >= 90) {
    achievements.push({
      ...
      icon: "🎯",
      description: "Maintained smooth flight",
    });
  }
