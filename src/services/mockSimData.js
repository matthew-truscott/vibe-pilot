...
export const generateMockFlightData = () => {
  const now = Date.now()
  ...
  flightData.verticalSpeed = Math.max(-2000, Math.min(2000, flightData.verticalSpeed))
  
  flightData.latitude += (Math.sin(flightData.heading * Math.PI / 180) * flightData.airspeed * deltaTime) / 3600000
  flightData.longitude += (Math.cos(flightData.heading * Math.PI / 180) * flightData.airspeed * deltaTime) / 3600000
  
  flightData.gForce = 1 + Math.abs(flightData.verticalSpeed) / 10000 + Math.abs(flightData.roll) / 100
  
  flightData.engine1RPM = 2200 + flightData.throttle * 4 + (Math.random() - 0.5) * 50
  flightData.engine1Temp = 150 + flightData.throttle * 0.5 + (Math.random() - 0.5) * 5
  
  flightData.fuelQuantity = Math.max(0, flightData.fuelQuantity - deltaTime * 0.01)
